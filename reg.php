...
if ($_GET['action']=='reged'){
   ...
   $res=@mysqli_query($conn,$check) or exit (mysqli_error());
	...
	echo "<script type='text/javascript'>alert('sorry ,your account has been registered');history.back();</script>";
  }
  else{
	  
	    $mm=md5($_clean['password']);
	  move_uploaded_file($_FILES["file"]["tmp_name"], "upload/" . $_FILES["file"]["name"]);
    $sql="insert into users (yhm,mm,tel,email,tx) values ('{$_clean['username']}','$mm','{$_POST['tel']}','{$_POST['email']}','{$_FILES['file']['name']}')" ;	
 ...
 <br>
      <table width="482" border="0" align="center">
        <tr>
         ...
         <tr>
          <td width="86" height="29">photo: </td>
          <td width="386"><input type="file" name="file" id="file" /></td>
        </tr> 
